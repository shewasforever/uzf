...
class Index extends Common{
    ...
    $cates = model('cate')->where('p_id','eq',0)->order('ca_sort desc')->select();
                //报单专区
    $map1[] = ['ca_id','=',11];
    //$map1[] = ['pd_status','=',3];
    $firstorder_goods = $Product->getListNpg($map1, '', '');
                //热门推荐
    //$map2[] = ['pd_status','=',5];
    $map2[] = ['ca_id','=',10];
    $Repeat_goods = $Product->getListNpg($map2, '', '');
                //首页显示
    $map3[] = ['ca_id','=',9];
    $preference_goods = $Product->getListNpg($map3, '', '');
                //热门大图
    //$map4[] = ['pd_status','=',6];
                //$bigrefer = $Product->getListNpg($map4, '', '');
    //$bigrefer = model('Product')->where($map4)->find();
                //halt($refers);
                //轮播图
    $shuffling = cache('setting')['shuffling_figure'];
    $array = explode(',', $shuffling);
    $newslists = model('Notice')->order('add_time desc')->field('id , add_time , title ')->find();
    $newslist = $newslists['title'];
    $data['shuffling'] = $array;
    $data['cates'] = $cates;
                //$data['word'] = '洗发乳';
    $data['firstorder_goods'] = $firstorder_goods;
    $data['Repeat_goods'] = $Repeat_goods;
    //$data['bigrefer'] = $bigrefer;
    $data['preference_goods'] = $preference_goods;
...
public function keyword(){
    ...
    $list['newest'] = $newest;$list['hotest'] = $hotest;$this->s_msg(null,$list);
}
public function search()
{
    ...
    $this->order = 'pd_price asc';
    }elseif ($by == 4) {$this->order = 'pd_price desc';
    }else{
    $this->order = '';
    }
    $Product = new ProductLogic;
...
public function cate()
{
    $Product = new ProductLogic;
    $cate_m = model('Cate');
    $map2[] = ['pd_status','=',6];$refers = model('Product')->where($map2)->find();

    $cates = $cate_m->where('p_id',0)->order('ca_sort desc')->select();foreach ($cates as $k => $v) {
...
public function newsDetail(){    //通告详情
    $id = input('post.id');
...
public function shopDetail(){
    ...
    if(input('post.ca_id') != ""){
        ...
        $ch_id = db('Cate')->where('p_id',$ca_id)->field('id')->select();
            //halt($ch_id);
        $all_id = array_column($ch_id,'id');
        array_push($all_id, $ca_id);$this->map[] = ['ca_id', 'in', $all_id];}  $this->map[] = ['pd_status','>',1];
